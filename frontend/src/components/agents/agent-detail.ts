...
if (!customElements.get('ff-agent-detail')) {
  ...
  export class AgentDetail extends LitElement {
    ...
    render() {
      if (this.isLoading) {
        return html`<div class="loading">Agent-Details werden geladen...</div>`;
      }
      return html`<h2>Agent-Details für ID: ${this.agentId}</h2>`;
    }
  }
}
...
async _loadData() {
  try {
    ...
    if (isDemoMode()) {
      ...
      // const transfers = await transferService.getAgentTransfers(this.agentId);
      // const tokens = await tokenService.getAgentTokens(this.agentId);
      // this.agent = agent;
...
_formatTokenStatus(status: string): string {
  switch (status) {
...
async _renewToken(tokenId: string) {
...
async _revokeToken(tokenId: string) {
